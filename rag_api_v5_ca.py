"""
RAG API v5_ca - SemanticSplitterNodeParser con documentos en catalán (sin pipeline de traducción):
- Igual que v5 pero indexa *_ca.txt directamente
- Pregunta y respuesta en catalán (sin traducción intermedia)
- Permite comparar con v5 (pipeline Ca→En→Ca) para medir el impacto de la traducción
- Puerto: 5007
- Tabla pgvector: llamaindex_semantic_chunks_ca
"""
...
def get_vector_store():
    return PGVectorStore.from_params(
        **DB_CONFIG,
        table_name="llamaindex_semantic_chunks_ca",  # tabla propia para v5_ca
        embed_dim=EMBED_DIM,
    )
...
def count_rows():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT COUNT(*) FROM data_llamaindex_semantic_chunks_ca")
        n = cur.fetchone()[0]
        cur.close()
        conn.close()
        return n
...
def rebuild():
    ...
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cur = conn.cursor()
        cur.execute("TRUNCATE TABLE data_llamaindex_semantic_chunks_ca")
        conn.commit()
        cur.close()
        conn.close()
    except Exception as e:
...
if __name__ == "__main__":
    print("Construyendo índice catalán con SemanticSplitterNodeParser...")
    _index = build_index()
