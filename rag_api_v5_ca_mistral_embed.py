"""
RAG API v5_ca_mistral_embed - SemanticSplitterNodeParser con documentos en catalán
usando mistral:7b-instruct-q4_0 como modelo de embeddings (en lugar de nomic-embed-text):
- Indexa *_ca.txt directamente (sin pipeline de traducción)
- Embeddings de 4096 dims generados por mistral (modelo generativo, no especializado)
- Permite comparar calidad de retrieval vs nomic-embed-text (768 dims, especializado)
- Puerto: 5008
- Tabla pgvector: llamaindex_semantic_chunks_ca_mistral
"""
...
EMBED_DIM = 4096  # mistral:7b produce embeddings de 4096 dims
EMBED_MODEL_OVERRIDE = os.getenv("LLM_MODEL", "mistral:7b-instruct-q4_0")  # usar mistral como embedding
...
def get_vector_store():
    return PGVectorStore.from_params(
        **DB_CONFIG,
        table_name="llamaindex_semantic_chunks_ca_mistral",  # tabla propia, 4096 dims
        embed_dim=EMBED_DIM,
    )
...
def count_rows():
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT COUNT(*) FROM data_llamaindex_semantic_chunks_ca_mistral")
        n = cur.fetchone()[0]
        cur.close()
        conn.close()
        return n
...
def build_index():
    embed_model = OllamaEmbedding(
        model_name=EMBED_MODEL_OVERRIDE,  # mistral como embedding model
        base_url=OLLAMA_URL,
    )
...
def rebuild():
    ...
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cur = conn.cursor()
        cur.execute("TRUNCATE TABLE data_llamaindex_semantic_chunks_ca_mistral")
        conn.commit()
        cur.close()
        conn.close()
    except Exception as e:
...
if __name__ == "__main__":
    print("Construyendo índice catalán con SemanticSplitterNodeParser (mistral embeds)...")
    _index = build_index()
    print(f"Modelo LLM: {LLM_MODEL} | Embeddings: {EMBED_MODEL_OVERRIDE} ({EMBED_DIM} dims)")
    print(f"SemanticSplitterNodeParser: breakpoint_percentile={BREAKPOINT_PERCENTILE}")
    print("Modo: catalán directo · embeddings con mistral:7b-instruct-q4_0")
